...
if (!in_array($mode, $modes)) {
	$return['state'] = 'ERROR';
	$return['description'] = TranslateUtility::assureLabel('eid.error.mode', 'faq', 'Es wurde kein Modus gesetzt. Bitte kontaktieren Sie den Administrator!');
} else {
	...
	if (in_array($question, $ids)) {
		$return['state'] = 'ERROR';
		$return['description'] = TranslateUtility::assureLabel('eid.error.multivote', 'faq', 'Sie haben f&uuml;r diese Frage bereits abgestimmt!');
		...
		if (!is_array($row)) {
			$return['state'] = 'ERROR';
			$return['description'] = TranslateUtility::assureLabel('eid.error.question', 'faq', 'Keine g&uuml;ltige Frage ausgew&auml;hlt!');
		} else {
			$ids[] = $row['uid'];
