	...
	public function findByTeaserConfiguration(array $topQuestions, array $categories, $limit) {
		...
		if ($limit > 0) {
			...
			$rows = HelperUtility::getDatabase()
			                     ->exec_SELECTgetRows($t . '.*', $t . ',tx_hdnet_faq_mm_question_questioncategory', $t . '.uid=tx_hdnet_faq_mm_question_questioncategory.uid_local AND tx_hdnet_faq_mm_question_questioncategory.uid_foreign IN (' . implode(',', $categories) . ')' . $topExclude . $pageRepository->enableFields($t), $t . '.uid', 'RAND()', $limit);
			foreach ($rows as $row) {
